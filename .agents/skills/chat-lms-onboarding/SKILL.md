...
3. setup 이 `GWS_CLIENT_MISSING` 을 내면 (자체 클라이언트가 필요한 경우):
   ...
   사용자는 Google 로그인만. 순서: console.cloud.google.com → 프로젝트 생성(없어도 됨,
   여기서 새로 만듦; 이름만, 결제 불필요) → "API 및 서비스 > 라이브러리"에서 4종 사용
   설정: Google Calendar API · Google Sheets API · Google Drive API · Gmail API →
   "OAuth 동의 화면" 구성(외부, 앱 이름만; **게시 상태를 '프로덕션'으로** — 테스트
   모드면 7일마다 재로그인 필요) → "사용자 인증 정보 > OAuth 클라이언트 ID >
