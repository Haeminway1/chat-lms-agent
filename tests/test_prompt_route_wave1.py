...
def _route_pack_v2(
    *,
    ...
    return {
        ...
        "id": pack_id,
        "bucket": "trigger",
        "summary": "lesson alias route",
        "required_tokens": list(required),
        "any_tokens": list(any_tokens),
        "first_command": "python -m chat_lms_agent side-panel lesson open-plan --json",
        "then_command": "python -m chat_lms_agent side-panel lesson ensure-server --json",
        "fallback_command": "python -m chat_lms_agent doctor --json",
        "must_not": ["do not create a new HTML report for this request"],
        "time_budget_ms": 5000,
    }
