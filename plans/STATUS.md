...
| `gws-integration-plan.md` | Google Workspace: onboarding OAuth setup, calendar/sheets/drive/gmail-send CLI, approval-gated send, anti-browser-fallback routes | **Waves 1–3 implemented 2026-06-11** — standard-library only (no `[gws]` extra needed; deviation from plan, core stays dependency-free). Remaining: Wave 0 owner action (create the OAuth client, drop it at `~/.chat_lms_agent/google_client.json` or embed), Wave 4 composite flows (word-test publish chain, side-panel schedule view) |
