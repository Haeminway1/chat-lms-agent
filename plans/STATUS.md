...
| `prompt-intent-routing-and-lesson-panel-plan.md` | Natural-language prompt routing to route packs/catalog plus lesson panel CLI/runtime/private bootstrap guarantees | **Completed 2026-06-12** — Waves 1-4 shipped: shared resolver, route-pack-v2, NO_MATCH catalog, lesson route/CLI/runtime assets, doctor/bootstrap guarantees, and generalized hydrate guidance |
| `side-panel-design-system-plan.md` | Display-spec lint, Toss-style design-system data, generation quarantine, dual-fixture verifier, verifier-backed promotion, and compliant lesson-panel template | **Completed 2026-06-13** — Waves D1-D4 shipped; promoted design viewers now require lint+verify evidence, install with backup, and expose doctor evidence-age repair rows |
| `external-integrations-sms-kakao-plan.md` | "External Integration Module" framework (generalize gws/classcard) + Solapi SMS + KakaoTalk all-tiers (free self-built chatbot inbound + admin-center friend messages; reseller-pluggable 알림톡/친구톡 API) | **Pending — designed/researched 2026-06-11; decisions resolved** (harness for many instructors → build all tiers, free-first, reseller-pluggable not Solapi-locked; 알림톡 reseller-mandated by Kakao policy = no free path). No owner gate — all waves can proceed |
| `kakao-channel-automation-design.md` | classcard-pattern headless automation of the channel admin center: Protocol-abstracted page, selector calibration pack, checkpoint/resume, shared approval gate | **Skeleton built + committed 2026-06-11** (782a9f2): K0-K2/K5 done, K4 partial (statistical summary). **Remaining: K3 live Playwright driver — needs a live calibration session, blocks all real sending — + K6 polish (model summary, quota tracking)** |
| `session-transcript-logging-plan.md` | Automatic per-session Codex rollout ingest into a private, owner-facing review log (`session_ledger.py` + `session-log` CLI); native `notify` + detached SessionStart triggers; redaction + learner-PII pseudonymization; host-neutral transcript locator | **Completed 2026-06-17** — ingest/list/show/export/status/enable/disable shipped; adversarial-QA hardened (credential-value redaction, full event-type capture, never-raises, byte-offset perf); see `docs/session-logging.md` |
| `gws-integration-plan.md` | Google Workspace: onboarding OAuth setup, calendar/sheets/drive/gmail-send CLI, approval-gated send, anti-browser-fallback routes | **Waves 1–3 implemented 2026-06-11** — standard-library only (no `[gws]` extra needed; deviation from plan, core stays dependency-free). Toss-style setup: embedded-default client slot (`EMBEDDED_CLIENT_ID`) + `gws client install` auto-detects the downloaded client JSON; the onboarding skill directs the agent to drive the one-time console flow via browser (user only logs in; publishing status must be In production). Owner console flow completed live in the Codex runtime (2026-06-11, ~5.5min agent-driven, one-time) and the product client is embedded — end users now see only the consent screen (~30s). Remaining: Wave 4 composite flows (word-test publish chain, side-panel schedule view) |
